...
int snakeBodyX[80], snakeBodyY[80]; 
int snakeLength = 3;
const int maxSnakeLength = 10; 
int applesEaten = 0;
...
void loop() {
  ...
  if (foodVisible && millis() - foodSpawnTime >= 7000) { 
    foodVisible = false;
  }
...
void resetGame() {
  ...
  snakeBodyX[0] = snakeX;       
  snakeBodyY[0] = snakeY;
  snakeBodyX[1] = snakeX - 1;   
  snakeBodyY[1] = snakeY;
  snakeBodyX[2] = snakeX - 2;   
  snakeBodyY[2] = snakeY;
...
void generateFood() {
    ...
    do {
        ...
        for (int i = 1; i <= 3; i++) {
            ...
            if (direction == 0) checkX = (snakeX + i) % 20;  
            if (direction == 1) checkY = (snakeY + i) % 4;   
            if (direction == 2) checkX = (snakeX - i + 20) % 20;  
            if (direction == 3) checkY = (snakeY - i + 4) % 4;    

            if (foodX == checkX && foodY == checkY) {
                inFront = true;
                break;
            }
        }
    } while (isOnSnake(foodX, foodY) || inFront);
...
void generateBomb() {
    ...
    } while (isOnSnake(bombX, bombY) || inFront);
    bombVisible = true;
